            ...
            MongoClient.connect(uri, function(err, db) {
                ...
                dbo.collection("Users").find({_id : ObjectId(tag)}).toArray(function(err, result) {
                    if (err) throw err;
                        ...
                        return response.send([{Tag: tag, NomeUtente: result[0].NomeUtente, Avatar: result[0].Avatar, DataAccount: result[0].DataAccount, Amici: result[0].Amici, MiSeguono: result[0].MiSeguono}]);
        ...
        MongoClient.connect(uri, function(err, db) {
            ...
            dbo.collection("Users").find({Email: myemail, Password: mypass}).toArray(function(err, result) {
                ...
                dbo.collection("Users").find({_id : ObjectId(userid)}).toArray(function(err, resultuser) {
                    ...
                    var oldMiSeguono;
                    var newMiSeguono;
                    if(resultuser[0].MiSeguono[0]._id != undefined) {
